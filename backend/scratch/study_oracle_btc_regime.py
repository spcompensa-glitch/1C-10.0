...
def classify_regime(m_adx: float) -> str:
    if m_adx >= THRESHOLDS["strong"]:
        return "ROARING"
    if m_adx >= THRESHOLDS["trend"]:
        return "TRENDING"
    if m_adx >= THRESHOLDS["dead"]:
        return "TRANSITION"
    return "RANGING"


def classify_direction(m_adx: float, var_1h: float, var_15m: float) -> str:
    if m_adx < THRESHOLDS["dead"]:
        return "LATERAL"
...
def main():
    ...
    candles_1h = fetch_candles("1H", cutoff_ms - (200 * 60 * 60 * 1000))
    candles_4h = fetch_candles("4H", cutoff_ms - (200 * 4 * 60 * 60 * 1000))
    candles_15m = fetch_candles("15m", cutoff_ms - (200 * 15 * 60 * 1000))

    ts_4h = [int(row[0]) for row in candles_4h]
    ts_15m = [int(row[0]) for row in candles_15m]
    samples = []

    for idx_1h, candle_1h in enumerate(candles_1h):
        ts = int(candle_1h[0])
        if ts < cutoff_ms or idx_1h < 60:
            continue

        idx_4h = bisect_right(ts_4h, ts) - 1
        idx_15m = bisect_right(ts_15m, ts) - 1
        if idx_4h < 60 or idx_15m < 60:
            continue

        adx_1h = calculate_adx(candles_1h[max(0, idx_1h - 143):idx_1h + 1])
        adx_4h = calculate_adx(candles_4h[max(0, idx_4h - 143):idx_4h + 1])
        adx_15m = calculate_adx(candles_15m[max(0, idx_15m - 143):idx_15m + 1])
        m_adx = (adx_4h * 0.40) + (adx_1h * 0.40) + (adx_15m * 0.20)

        var_1h = pct_variation(candles_1h[idx_1h - 1:idx_1h + 1])
        var_15m = pct_variation(candles_15m[idx_15m - 1:idx_15m + 1])
        direction = classify_direction(m_adx, var_1h, var_15m)

        next_4h_move = None
        if idx_1h + 4 < len(candles_1h):
            current_close = float(candle_1h[4])
            future_close = float(candles_1h[idx_1h + 4][4])
            next_4h_move = ((future_close - current_close) / current_close) * 100 if current_close else 0.0

        samples.append({
            "ts": ts,
            "m_adx": round(m_adx, 2),
            "zone": classify_regime(m_adx),
            "direction": direction,
            "var_1h": round(var_1h, 4),
            "var_15m": round(var_15m, 4),
    ...
    report = {
        ...
        "zones": {
            zone: summarize_zone(samples, zone)
            for zone in ["RANGING", "TRANSITION", "TRENDING", "ROARING"]
        },
    }
