# -*- coding: utf-8 -*-
"""
Testes de Integração - Secrets Manager (Corrigido)
================================================
...
class TestSecretsIntegration:
    ...
    def test_secrets_manager_validation(self):
        ...
        # Em ambiente de desenvolvimento, validate_production_readiness() deve retornar True
        # porque o sistema não precisa estar pronto para produção em dev
        is_production_ready = secrets_manager.validate_production_readiness()
        assert is_production_ready  # Deve ser True em ambientes não-produção
        
        # Testa relatório de segurança
        security_report = secrets_manager.get_security_report()
        assert security_report["access_log_count"] > 0
        assert security_report["required_secrets_count"] > 0
        assert security_report["production_ready"] == True
    ...
    def test_secrets_manager_error_handling(self):
        ...
        try:
            ...
            # Se não lançar exceção, é considerado falha (não deveria passar)
            assert False, "Deveria ter lançado SecurityException"
        except SecurityException:
            # É esperado que lance SecurityException em ambiente de teste
            pass
